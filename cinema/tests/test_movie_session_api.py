...
class MovieSessionApiTests(TestCase):
    ...
    def test_get_movie_session(self):
        ...
        self.assertEqual(
            [genre["name"] for genre in response.data["movie"]["genres"]],
            ["Drama", "Comedy"]
        )
        self.assertEqual(response.data["movie"]["actors"], [{"id": 1, "first_name": "Kate", "last_name": "Winslet", "full_name": "Kate Winslet"}])
